...
| 9     | Backfill execution — DONE (auto-track). Migrations applied to remote D1, Google secrets set, prod token seeded, smoke-test passed (216k Gmail messages accessible). Calendar full pull: 11030 scanned, 59 matched, 57 inserted. Gmail full pull: 694 scanned, 20 parsed, 665 source rows captured. **61 canonical events live in prod** across 11 years (2016–2026): Mariners 28, NCAAF 10, NFL 6, WNBA 3, NCAAB 1, concerts 13. 660 Gmail sources sit pending for Phase 3.5 follow-up parsers (Ticketmaster/AXS/Vivid). Manual UW imports (9.6/9.7) gated on user curation. |
