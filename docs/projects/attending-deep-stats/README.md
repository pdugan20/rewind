...
| Decision                                                                                                                                                                                      | Why                                                                                                                                                                                                                                                                                                                    |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase 1 ships **both** a `team` substring filter (against `event_data.home_team.name` / `away_team.name`) and a `team_id` integer filter (against `event_data.home_team.id` / `away_team.id`) | Confirmed during planning that `home_team`/`away_team` are stored as objects `{ id, name }`, not strings. Name substring serves natural-language ("mariners"); integer id is a stable join key for code/portfolio consumers. Cheap to ship both — same JSON-extract code path. See [DESIGN.md](./DESIGN.md).           |
...
| Game card consumes existing `get_attended_event` structuredContent unchanged                                                                                                                  | Confirmed during planning that the existing response carries everything the card needs: full event_data with per-inning `linescore` for MLB, `players` array with per-player batting/pitching/fielding lines + `notable` flag + photos, full `venue`, full `tickets`. No API-shape changes required for Phase 3.       |
...
- **[Sports box-score parity](../sports-boxscore-parity/)** — NFL / NBA / WNBA / NCAAF / NCAAB / MLS per-player stat lines via ESPN summary endpoints. Scoped now, execution gated on the Phase 4 checkpoint here. Ships the underlying data; Tier 2 endpoint extensions to non-MLB leagues are a small follow-up after that project lands.
...
| [DESIGN.md](./DESIGN.md)   | Source-of-truth data shapes (`event_data`, `batting_line`, `pitching_line`), aggregation gotchas, coverage numbers |
